...
import BikeSimDecomp2 as BikeSim
...
def runDay(mbm, statMapGen, durations, simMethod, startTime, numIntervals,
    ...
    return (gc.getFailedEnds(),
            gc.getFailedStartsAM(),
            gc.getFailedStartsPM(),
            gc.getAlterEndAM(),
            gc.getAlterEndPM(),
            gc.getFailedStartsStationAM(),
            gc.getFailedStartsStationPM(),
            gc.getAlterEndsStationAM(),
            gc.getAlterEndsStationPM(),
            gc.getTotalTrips())
...
def statMapTestSol(level, sM):
    # Put in a starting allocation
    stations = {}
    for sid in level.keys():
        stations[sid] = BikeSim.Station(sid, sM[sid]['name'], sM[sid]['capacity'], level[sid], 0, sM[sid]['ords'])
    return stations
