...
class TripGenerator(object):
    ...
    def getDuration(self, s1, s2, t, ind=None):
        ...
        # random number cycle'''
        # if ind is None:
        #     # duration = int(round(numpy.exp(slope*numpy.log(est) + numpy.random.normal(0,sigma) + intercept)/60.0))
        #     duration = int(round(est**slope*numpy.exp(intercept)*numpy.random.lognormal(0,sigma)/60.0))
        # else:
        #     state = numpy.random.get_state()
