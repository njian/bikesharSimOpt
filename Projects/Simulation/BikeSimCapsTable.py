...
class Trip(object):
    ...
    def failEnd(self, ):
        ...
        if len(self.visitedEnds) < 3: # update the end location and time of this trip
...
class Station(object):
    ...
    def tripIn(self, trip):
        # if len(trip.visitedEnds) == 0:
        self.arrowList.append(1)
...
class GlobalClock(object):
    ...
    def getAlterEnd(self, ):
        # count = 0
        # for x in self.trips:
        #     count += len(x.visitedEnds)
        # # print count, len(filter(lambda x: x.state is Trip.ENDFAILED, self.trips))
        # return count
        # # return len(lambda x: len(x.visitedEnds), self.trips)
    ...
    def getFailedStartsStationCount(self, ):
        failedStartTrips = filter(lambda x: x.state is Trip.STARTFAILED, self.trips)
        sdic = {}
        for i in failedStartTrips:
            if sdic.has_key(i.startLoc):
                sdic[i.startLoc] += 1
            else:
                sdic[i.startLoc] = 1
        return sdic
    ...
    def getAlterEndsStationCount(self, ):
        failedEndTrips = filter(lambda x: x.failedEndSid is not None, self.trips)
        sdic = {}
        for i in failedEndTrips:
            if sdic.has_key(i.failedEndSid):
                sdic[i.failedEndSid] += 1
            else:
                sdic[i.failedEndSid] = 1
        return sdic
    ...
    def getArrowDict(self, ):
        return self.arrowDict
    ...
    def tick_original(self, minTrips):
        ...
        while len(self.heap) > 0 and self.heap[0] < self.time:
            ...
            if trip.endLoc is not None and self.statMap.has_key(trip.endLoc): # mismatch statMap: may not have endLoc
                ...
                if tin is not None: failedEndTrips.append(tin)

        # Then we make new end point trips happen
        for trip in failedEndTrips:
