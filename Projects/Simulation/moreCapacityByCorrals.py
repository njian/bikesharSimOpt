...
def randomSearch(sM, mbm, durations, fileName, startTime, numIntervals, config):
    ...
    maxCap = {}
    for sid in sM.keys():
        maxCap[sid] = 80 if sid == 477 or sid == 519 or sid == 432 or sid == 511 else 60
        ...
        try:
            ...
            for m in range(nMatch):
                ...
                while TrySolnLvl[mBike[0]] - nMove < 0:
                    ...
                    mBike = mBikeCost[chosenInd[1]]
                while TrySolnCaps[pCap[0]] + nMove > maxCap[pCap[0]]:
                    pCapCost.pop(chosenInd[2])
                    pCap = pCapCost[chosenInd[2]]
                while TrySolnCaps[mCap[0]] - nMove < minCap:
                    mCapCost.pop(chosenInd[3])
                    mCap = mCapCost[chosenInd[3]]
                while TrySolnLvl[pBikeCap[0]] + nMove > TrySolnCaps[pBikeCap[0]] or TrySolnCaps[pBikeCap[0]] + nMove > maxCap[pBikeCap[0]]:
...
if __name__ == '__main__':
    ...
    logging.info('Purpose of the run: restart from CTMCVaryRate6to24_15xid12CapsTableRnCaps without maxCap = 80 at sid 477, 519, 432')
