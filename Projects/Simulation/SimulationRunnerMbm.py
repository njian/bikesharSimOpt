...
def runDay(rep, mbm, statMapGen, listID, sctPlt = None, nsamples=1, tgenMult=1,
           sctPltTimes = None, vehicles=[]):
    sM = statMapGen
    gc = BikeSimMbm.GlobalClock(sM, BikeSimMbm.TripGenerator(sM, mbm, tgenMult), listID, 
    ...
    return (gc.getFailedEnds(),
            gc.getFailedStarts(),
            gc.getAlterEnd(),
            gc.getFailedStartsStation(),
            gc.getAlterEndsStation(),
            gc.getTotalTrips())

    '''return (gc.getTripOk(), gc.getFailedEnds(),
            gc.getAlterEnd(), gc.getOutageMins(),
            gc.getTotalTrips(), gc.getEmptyMins(),
            gc.getFullMins(), gc.getCompletedTrips(),
            gc.getFailedStarts())'''
...
def statMap50GenDepots():
    ...
    # Add depots
    sM[0] = BikeSimMbm.Station(0, "depot 0", 400, 200, 0, (0,0))
    sM[1] = BikeSimMbm.Station(1, "depot 1", 400, 200, 0, (0,0))
...
def statMapTestSol(level, d):
    # Put in an allocation
    # total = 0
    sM = {}
    for sid in level.keys():
        sM[sid] = BikeSimMbm.Station(sid, d[sid]['name'], d[sid]['capacity'], level[sid], 0, d[sid]['ords'])
        # total += stat.level
    # print "Total bikes test solution:", total
    # print "Total stations test solution:", total2
    return sM
...
def getMorningTrailerRoutes():
    ...
    for s,e in Config.morningPairs:
        res.append( BikeSimMbm.BikeTrailerRouter( (s, e), 12, 6*60, 11*60 ) )
    return res
...
def getEveningTrailerRoutes():
    ...
    for s,e in Config.eveningPairs:
        res.append( BikeSimMbm.BikeTrailerRouter( (s, e), 12, 13*60, 20*60 ) )
    return res
...
def getVehicles():
    ...
    for btr in getMorningTrailerRoutes():
        vehicles.append( BikeSimMbm.Trailer(btr) )
...
def testVehicles2(rep, level, ords, mbm, listID):
    data = runDay(rep, mbm, statMapTestSol(level, ords), listID, sctPlt = None, nsamples=1,  tgenMult=1, sctPltTimes = None, vehicles=[])
    # columns = ["Successful Trips", "Failed Ends", "Different Ends",
    #            "Outage Minutes", "Total Trips", "Empty Mins", "Full Mins",
    #            "Completed Trips", "Failed Starts", "Most Failed Starts Station", "Most Failed Ends Station"]
    return data
