...
def cheatSearch(ctmc, sM, mbm, durations, fileName, startTime, numIntervals, config):
    ...
    while True: # Stopping criteria
        try:
            ...
            for m in range(nMatch):
                ...
                while TrySolnLvl[pBike[0]] + nMove > TrySolnCaps[pBike[0]] or TrySolnLvl[pBike[0]] < ctmc[pBike[0]]['bikes']:
                    pBikeCost.pop(chosenInd[0])
                    pBike = pBikeCost[chosenInd[0]]
                while TrySolnLvl[mBike[0]] - nMove < 0 or TrySolnLvl[mBike[0]] > ctmc[mBike[0]]['bikes']:
                    mBikeCost.pop(chosenInd[1])
                    mBike = mBikeCost[chosenInd[1]]
                while TrySolnCaps[pCap[0]] + nMove > maxCap or TrySolnLvl[pCap[0]] < ctmc[pCap[0]]['capacity']:
                    pCapCost.pop(chosenInd[2])
                    pCap = pCapCost[chosenInd[2]]
                while TrySolnCaps[mCap[0]] - nMove < minCap or TrySolnCaps[mCap[0]] > ctmc[mCap[0]]['capacity']:
                    mCapCost.pop(chosenInd[3])
                    mCap = mCapCost[chosenInd[3]]
                while (TrySolnLvl[pBikeCap[0]] + nMove > TrySolnCaps[pBikeCap[0]] or TrySolnCaps[pBikeCap[0]] + nMove > maxCap) or \
                      TrySolnLvl[pBikeCap[0]] < ctmc[pBikeCap[0]]['bikes'] or TrySolnCaps[pBikeCap[0]] < ctmc[pBikeCap[0]]['capacity']:
                    pBikeCapCost.pop(chosenInd[4])
                    pBikeCap = pBikeCapCost[chosenInd[4]]
                while (TrySolnCaps[mBikeCap[0]] - nMove < minCap or TrySolnLvl[mBikeCap[0]] - nMove < 0) or \
                      TrySolnLvl[mBikeCap[0]] > ctmc[mBikeCap[0]]['bikes'] or TrySolnCaps[mBikeCap[0]] > ctmc[mBikeCap[0]]['capacity']:
...
if __name__ == '__main__':
    ...
    nMatch = 1 # max allowed matches of station pairs to move bike/dock
    ...
    fileName = (solutionName + "id" + str(fileInd) + "cheating")
    ...
    logging.info('Purpose of the run: cheat search from ending solution of avgAlloc towards ctmc')
    ...
    capacity = cPickle.load(open('./data/AverageAllocationid30CapsTableRnCapsCapacity.p', 'r'))
    level = cPickle.load(open('./data/AverageAllocationid30CapsTableRnCapsLevel.p', 'r'))
    for sid in sM.keys():
    ...
    # capacity = cPickle.load(open('./data/CTMCVaryRate6to10_15xid77CapsTableRnCapsCapacity.p', 'r'))
    # level = cPickle.load(open('./data/CTMCVaryRate6to10_15xid77CapsTableRnCapsLevel.p', 'r'))
    # for sid in ctmc.keys():
    #     ctmc[sid]['bikes'] = level[sid]
    #     ctmc[sid]['capacity'] = capacity[sid]
    ctmc[-1000] = {'bikes':-1000, 'capacity':-1000, 'ords':(0,0), 'name':"null"}
    ...
    cheatSearch(ctmc,sM,mbm,durationMult,fileName,startTime,numIntervals,config)
