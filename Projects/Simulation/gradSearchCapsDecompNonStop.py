        ...
        try:
            ...
            while 3*nswap-totalBikesTaken > 0: # if taken < 3*nswap
                ...
                while TrySolnLvl[sidC3] - nswap < 0:
                    ...
                    if ct >= 50:
                        ...
                        print "in while 1! tried %i loops" % ct
                    sidC3 = random.choice(statCList)
            ...
            for sid in failedAddSid:
                ...
                while TrySolnLvl[sidC4] - nswap < 0:
                    ...
                    if ct >= 50:
                        statCList = returnTopStatCList(statC, capacity, 100, nswap, minCap)
                        print "in while 2! tried %i loops" % ct
            ...
            while totalBikesTaken - totalBikesGiven > 0: # still has more bikes to allocate, then find more sidC to give bikes if necessary
                ...
                while TrySolnLvl[sidC3] + nswap > TrySolnCaps[sidC3]:
                    ...
                    if ct >= 50:
                        statCList = returnTopStatCList(statC, capacity, 100, nswap, minCap)
                        print "in while 3! tried %i loops" % ct
                    sidC3 = random.choice(statCList)
                TrySolnLvl[sidC3] += nswap
                totalBikesGiven += nswap
                logger1.debug( (str(nswap) + " bikes given to sidC " + str(sidC3) + ',' + str(statCDict[sidC3])) )
...
if __name__ == '__main__':
    ...
    solutionName = 'CTMCVaryRate' + str(start) + "-" + str(end) + '_15x' 
    ...
    logging.info('gradSearchCapsDecompNonStop.py')
    # logging.info('Purpose of the run: checking the station categories')
    # logging.info('Purpose of the run: 1.5 mbm average allocation, reps = 30, new durations') # comments
    # logging.info('extension of CTMCVaryRate6-24_15xid89CapsDecompCapacity') # comments
    logging.info('CTMC nonstop')
    ...
    # level = cPickle.load(open('./outputsDO/CTMCVaryRate6-24_15xid89CapsDecompLevel.p', 'r')) # surface
    level = cPickle.load(open('./outputsDO/CTMCVaryRate6-24_15xid86CapsDecompLevel.p','r')) # office
    for s in sM.keys():
        sM[s]['bikes'] = level[s]
    # capacity = cPickle.load(open('./outputsDO/CTMCVaryRate6-24_15xid89CapsDecompCapacity.p', 'r')) # surface
    capacity = cPickle.load(open('./outputsDO/CTMCVaryRate6-24_15xid86CapsDecompCapacity.p','r')) # office
    for s in sM.keys():
        sM[s]['capacity'] = capacity[s]
