...
import BikeSimMbm as BikeSim
# import BikeSimDBBackend
# import Config
import numpy

def runDay(rep, mbm, statMapGen, durations, listID, simMethod, startTime, numIntervals,
            sctPlt = None, nsamples=1, tgenMult=1, sctPltTimes = None, vehicles=[]):
    sM = statMapGen
    gc = BikeSim.GlobalClock(startTime, sM, BikeSim.TripGenerator(sM, mbm, durations, tgenMult), listID,
    ...
    outage = {}
    for sid in sM.keys():
        outage[sid] = sum(sM[sid].getOutageMins())
...
def statMapTestSol(level, capacity, sM):
    ...
    for sid in level.keys():
        stations[sid] = BikeSim.Station(sid, sM[sid]['name'], capacity[sid], level[sid], 0, sM[sid]['ords'])
    return stations
...
def testVehicles2(rep, level, capacity, sM, mbm, durations, simMethod, startTime, numIntervals, listID):
    data = runDay(rep, mbm, statMapTestSol(level, capacity, sM), durations, listID, simMethod, startTime, numIntervals,
        sctPlt = None, nsamples=1,  tgenMult=1, sctPltTimes = None, vehicles=[])
    return data
